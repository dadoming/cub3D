...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/05/18 20:41:52 by dadoming          #+#    #+#             */
/*   Updated: 2023/05/18 20:42:59 by dadoming         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../includes/cub3D.h"

static void		load_elinfo(t_elinfo *elinfo, \
	char *file_line, int i, int option);
...
void	load_elinfo(t_elinfo *elinfo, char *file_line, int i, int option)
{
	...
	if (file_line[spaces + 2 + option] == '\n' || \
		...
		return ;
	elinfo[i].val = ft_substr(file_line, 2 + option + \
		spaces, ft_strlen(file_line) - 2 - option - spaces);
