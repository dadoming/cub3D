...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/05/18 20:34:02 by dadoming          #+#    #+#             */
/*   Updated: 2023/05/18 20:59:39 by dadoming         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_anim_list	*load_n_images(t_game *game, char *imagename, int number_of_images)
{
	...
	while (i <= number_of_images)
	{
		...
		frame.img = mlx_xpm_file_to_image(game->mlx, filename, \
		...
		frame.addr = mlx_get_data_addr(frame.img, \
			&frame.bits_per_pixel, &frame.line_length, &frame.endian);
...
{
	t_anim_list	*new_node;

	new_node = (t_anim_list *)malloc(sizeof(t_anim_list));
	if (new_node == NULL)
	{
...
void	insert_frame(t_anim_list **head, t_imgbuffer frame)
{
	...
	new_node = create_frame_node(frame);
	if (*head == NULL)
	{
		*head = new_node;
	}
